		...
		case $i in
			cf*)
				...
				if [ $? == 1 ] ; then
					...
					echo "<hr><h4>$i details</h4>${ERRAND_RESULT}" >> email/verbosetest.html
					echo "$i failed smoke test"
					FAILURE=true
				else
					printf "$i passed its Smoke-Test<br>" >> email/verboseresults.html
					echo "$i passed smoke test"
				fi
				;;
			hashicorp-enterprise-vault-consul|pivotal-mysql*|p-redis*|p-rabbitmq*|apmPostgres*|p-spring*)
			...
			# 		FAILURE=true
			# 	else
			# 		printf "<br>$i passed its Smoke-Test<br>" >> email/verboseresults.html
			# 		echo "$i passed smoke test"
			# 	fi
			# 	;;
			redislabs-service-broker*)
                # fog is on a newer version of the broker, in that one the errand name has changed to service-broker-smoke-test
				if [ "$PCF_ENV" == "fog" ]; then
                    ERRAND_RESULT=$(bosh -e $PCF_ENV -d $i run-errand service-broker-smoke-test)
                elif [ "$PCF_ENV" == "pxa" ]; then
                    ERRAND_RESULT=$(bosh -e $PCF_ENV -d $i run-errand service-broker-smoke-test)
                else
